# Buildings — техническая спецификация PIF
...
Этот документ описывает техническую организацию слоя `buildings` в Planetary Infrastructure Framework. Он фиксирует, как PIF анализирует здания и building-like объекты, включая capitals, branch office buildings, holdings и специальные ключи без префикса `building_`, раскладывает параметры по категориям, классифицирует modifier blocks по содержимому, создаёт доменные переменные и проверяет сохранение vanilla-поведения.

## Общая характеристика слоя

`buildings` — крупнейший слой планетарной инфраструктуры. Он включает не только обычные `building_*`, но и capitals, branch office buildings, overlord holdings, а также специальные building-like ключи без префикса `building_`.

Buildings соединяют почти все игровые области планетарной инфраструктуры: construction, cost/upkeep, jobs, planet state, country effects, lifecycle, upgrades, conversion, AI и branch/holding-specific mechanics. Поэтому PIF использует более детальную структуру категорий, чем для districts и zones, и отдельно классифицирует modifier-carrier blocks по фактическому содержимому.
...
| --- | --- | --- |
| regular | 366 | Обычные building objects, не capital, не branch office и не holding. |
| capital | 41 | Здания с `capital = yes` и `capital_tier`. |
| branch | 35 | Branch office buildings с `owner_type = corporate`. |
| holding | 30 | Overlord holdings с `owner_type = subject_holding` или ключом `holding_*`. |
| special | 18 | Building-like объекты без префикса `building_`. |
...
| --- | --- | --- | --- |
| METADATA | Root object | Identity, UI и engine-sensitive fields. | `category`, `owner_type`, `capital`, `capital_tier`, icons, descriptions, `base_buildtime`. |
| BUILDING_CONFIG | Inline script | Building set compatibility. | `building_sets`. |
| AVAILABILITY | Inline script | Условия строительства, видимости и лимиты. | `potential`, `allow`, `can_build`, `prerequisites`, `planet_limit`, `empire_limit`. |
| ECONOMY | Inline script | Стоимость, содержание и resource block building. | `resources`. |
| LIFECYCLE | Inline script | Состояние, upgrades, conversions и construction events. | `can_demolish`, `can_be_ruined`, `destroy_trigger`, `convert_to`, `upgrades`, `on_*`. |
...
| `pif_buildings_planet_state_variables.txt` | 354 | Housing, amenities, crime, stability, slots and defense state. |
...
Особенно важны `upgrades`, `convert_to`, repeated modifier blocks, lifecycle effects и порядок category inline calls.
...
Текущий результат: **490 / 490 OK**, failed: **0**.
...
| Параметр | Тип | Всего | Regular | Capital | Branch | Holding | Special | Назначение | Категория | Действие |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `additional_ai_weight` | number / scalar | 23 | 9 | 14 | 0 | 0 | 0 | Дополнительный вес для AI при оценке объекта. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `ai_estimate_without_unemployment` | block / value | 55 | 49 | 6 | 0 | 0 | 0 | Инструкция AI оценивать объект без учёта unemployment. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `ai_resource_production` | block / value | 55 | 54 | 0 | 1 | 0 | 0 | Подсказка AI о ресурсной роли объекта. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `ai_weight` | block / value | 33 | 0 | 0 | 33 | 0 | 0 | Блок веса AI для выбора объекта. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `ai_weight_coefficient` | number / scalar | 39 | 38 | 1 | 0 | 0 | 0 | Коэффициент веса AI. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `custom_storm_ai_weight` | block / value | 11 | 11 | 0 | 0 | 0 | 0 | Специальный AI-вес в контексте cosmic storms. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `exempt_from_ai_planet_specialization` | block / value | 24 | 24 | 0 | 0 | 0 | 0 | Исключение из AI planet specialization. | AI | Вынести в PIF-owned inline script соответствующей категории. |
| `allow` | trigger block | 262 | 208 | 18 | 8 | 18 | 10 | Условие доступности действия прямо сейчас. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `base_cap_amount` | number / scalar | 14 | 12 | 0 | 2 | 0 | 0 | Базовый cap amount для capped buildings. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `can_build` | yes/no | 260 | 209 | 41 | 0 | 0 | 10 | Разрешает или запрещает обычное строительство. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `district_limit` | number / scalar | 1 | 1 | 0 | 0 | 0 | 0 | Лимит, связанный с количеством districts. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `empire_limit` | number / scalar | 34 | 28 | 1 | 0 | 2 | 3 | Лимит объектов в империи. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `planet_limit` | number / scalar | 243 | 177 | 0 | 33 | 28 | 5 | Лимит объектов на планете. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `potential` | trigger block | 463 | 350 | 41 | 35 | 29 | 8 | Базовое условие существования или показа объекта. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `prerequisites` | list / block | 245 | 212 | 18 | 5 | 0 | 10 | Список технологий или требований разблокировки. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `show_in_tech` | atom / enum | 38 | 38 | 0 | 0 | 0 | 0 | Явная привязка показа объекта к технологии. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `show_tech_unlock_if` | trigger block | 164 | 141 | 11 | 2 | 0 | 10 | Условие показа объекта как tech unlock. | AVAILABILITY | Вынести в PIF-owned inline script соответствующей категории. |
| `building_sets` | list / block | 425 | 366 | 41 | 0 | 0 | 18 | Специализированный параметр building. | BUILDING_CONFIG | Вынести в PIF-owned inline script соответствующей категории. |
| `resources` | resources block | 462 | 346 | 33 | 35 | 30 | 18 | Экономический блок: cost, upkeep, produces и economic category. | ECONOMY | Вынести в PIF-owned inline script соответствующей категории. |
| `can_be_disabled` | yes/no | 44 | 14 | 30 | 0 | 0 | 0 | Разрешает disabled state для building. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `can_be_ruined` | yes/no | 78 | 42 | 36 | 0 | 0 | 0 | Разрешает переход building в ruined state. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `can_demolish` | yes/no | 158 | 117 | 41 | 0 | 0 | 0 | Разрешает или запрещает снос объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `convert_to` | list / block | 169 | 98 | 36 | 20 | 0 | 15 | Список targets для конверсии объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `destroy_trigger` | trigger block | 371 | 316 | 10 | 21 | 9 | 15 | Условие разрушения или автоматической конверсии объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_built` | effect block | 35 | 16 | 2 | 4 | 13 | 0 | Эффект при завершении строительства. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_destroy` | effect block | 33 | 19 | 0 | 3 | 11 | 0 | Эффект при уничтожении объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_enabled` | effect block | 5 | 5 | 0 | 0 | 0 | 0 | Эффект при включении объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_queued` | effect block | 101 | 93 | 5 | 0 | 0 | 3 | Эффект при постановке в очередь строительства. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_repaired` | effect block | 9 | 9 | 0 | 0 | 0 | 0 | Эффект при ремонте объекта. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `on_unqueued` | effect block | 101 | 93 | 5 | 0 | 0 | 3 | Эффект при удалении из очереди строительства. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `upgrades` | list / block | 188 | 147 | 31 | 0 | 0 | 10 | Цепочка улучшений building. | LIFECYCLE | Вынести в PIF-owned inline script соответствующей категории. |
| `auto_generate_description` | yes/no | 2 | 2 | 0 | 0 | 0 | 0 | Управляет автогенерацией описания эффектов. | METADATA | Оставить в корневом объекте. |
| `base_buildtime` | number / scalar | 454 | 346 | 25 | 35 | 30 | 18 | Базовое время строительства. | METADATA | Оставить в корневом объекте. |
| `capital` | yes/no | 41 | 0 | 41 | 0 | 0 | 0 | Флаг capital building. | METADATA | Оставить в корневом объекте. |
| `capital_tier` | number / scalar | 41 | 0 | 41 | 0 | 0 | 0 | Технический уровень capital building. | METADATA | Оставить в корневом объекте. |
| `category` | atom / enum | 490 | 366 | 41 | 35 | 30 | 18 | Категория объекта в vanilla-системе. | METADATA | Оставить в корневом объекте. |
| `custom_tooltip` | block / value | 37 | 10 | 0 | 27 | 0 | 0 | Дополнительный tooltip. | METADATA | Оставить в корневом объекте. |
| `desc` | block / value | 9 | 9 | 0 | 0 | 0 | 0 | Условное или прямое описание объекта. | METADATA | Оставить в корневом объекте. |
| `icon` | atom / enum | 163 | 107 | 19 | 4 | 30 | 3 | Иконка объекта. | METADATA | Оставить в корневом объекте. |
| `is_essential` | yes/no | 7 | 7 | 0 | 0 | 0 | 0 | Флаг essential building. | METADATA | Оставить в корневом объекте. |
| `owner_type` | atom / enum | 65 | 0 | 0 | 35 | 30 | 0 | Тип владельца для branch office или holding logic. | METADATA | Оставить в корневом объекте. |
| `position_priority` | number / scalar | 66 | 25 | 41 | 0 | 0 | 0 | Порядок отображения building. | METADATA | Оставить в корневом объекте. |
| `ruined_icon` | atom / enum | 44 | 39 | 5 | 0 | 0 | 0 | Иконка ruined-состояния. | METADATA | Оставить в корневом объекте. |
| `skip_automation_upgrading` | yes/no | 1 | 1 | 0 | 0 | 0 | 0 | Запрещает automatic upgrade logic. | METADATA | Оставить в корневом объекте. |
| `triggered_desc` | block / value | 112 | 88 | 6 | 3 | 12 | 3 | Условные описания и tooltips. | METADATA | Оставить в корневом объекте. |
| `planetary_ftl_inhibitor` | modifier / flag | 5 | 5 | 0 | 0 | 0 | 0 | Флаг planetary FTL inhibitor. | PLANET_STATE | Вынести в PIF-owned inline script соответствующей категории. |
| `country_modifier` | modifier / flag | 35 | 25 | 0 | 5 | 2 | 3 | Статический country modifier carrier. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `planet_modifier` | modifier / flag | 296 | 236 | 39 | 3 | 18 | 0 | Статический planet modifier carrier. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `system_modifier` | modifier / flag | 1 | 1 | 0 | 0 | 0 | 0 | System-level modifier carrier. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `triggered_country_modifier` | modifier / flag | 72 | 15 | 25 | 27 | 2 | 3 | Условный country modifier carrier. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `triggered_planet_modifier` | modifier / flag | 373 | 261 | 34 | 34 | 26 | 18 | Условный planet modifier carrier. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `triggered_planet_pop_group_modifier_for_all` | modifier / flag | 2 | 2 | 0 | 0 | 0 | 0 | Условный modifier для всех pop groups планеты. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |
| `triggered_planet_pop_group_modifier_for_species` | modifier / flag | 1 | 1 | 0 | 0 | 0 | 0 | Условный modifier для pop groups конкретного species. | content-based modifier category | Классифицировать block по содержимому; static modifier при необходимости нормализовать. |

## Особые случаи и технические предупреждения

- Нужно загружать все top-level objects из `common/buildings`, а не только `building_*`.
- `holding_*` и branch office buildings имеют отдельную структуру и не обязаны иметь `building_sets`.
- `building_citadel_uplink` сохраняет vanilla-дублирование `category`.
- `job_capital_trader_add` считается warning-level reference, а не failure.
